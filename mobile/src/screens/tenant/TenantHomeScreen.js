        ...
        >
          <View style={styles.invoiceTop}>
            ...
            <View style={[styles.badge, { backgroundColor: statusColors[currentInvoice.status]?.bg }]}>
              <Text style={[styles.badgeText, { color: statusColors[currentInvoice.status]?.text }]}>
      ...
      {notices.length === 0 ? (
        ...
        <View style={styles.list}>
          {notices.map((notice, index) => (
            <View
              key={notice.id}
              style={[styles.noticeRow, index === notices.length - 1 && { borderBottomWidth: 0 }]}
            >
...
const styles = StyleSheet.create({
  ...
  payButtonText: { fontFamily: 'Manrope_700Bold', fontSize: 15, color: colors.surface },
  sectionTitle: { marginTop: spacing.xl, marginBottom: spacing.sm },
  emptyState: { backgroundColor: colors.surface, borderRadius: 12, borderWidth: 1, borderColor: colors.border, padding: spacing.lg, marginTop: spacing.xs },
  list: { backgroundColor: colors.surface, borderRadius: 12, borderWidth: 1, borderColor: colors.border },
  noticeRow: { padding: spacing.md, borderBottomWidth: 1, borderBottomColor: colors.border },
  noticeHeader: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs },
  pinDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: colors.accent },
});